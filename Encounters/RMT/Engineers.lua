...
local Locales = {
    ["enUS"] = {
		...
		["debuff.electroshock_vulnerability_leave"] = "Electroshock LEAVE",
		["debuff.electroshock_vulnerability_return"] = "Electroshock Return",
        -- Casts
        ...
        if self.config.alerts.pillar.enable or self.config.sounds.pillar.enable then
            if nPercent < 20 then
                ...
                if sCurrentPlatform and sName == self.L[sCurrentPlatform] then
					if nPercent < 13 and nPercent < lastPillarHealth then
...
function Mod:OnBuffAdded(nId, nSpellId, sName, tData, sUnitName, nStack, nDuration)
    if nSpellId == DEBUFF_ELECTROSHOCK_VULNERABILITY then
        ...
        if tData.tUnit:IsThePlayer() then
			self.core:AddTimer("Vulnerability_leave_timer_"..tostring(nId), self.L["debuff.electroshock_vulnerability_leave"], 10, self.config.timers.vulnerability_leave)
			self.core:AddTimer("Vulnerability_return_timer_"..tostring(nId), self.L["debuff.electroshock_vulnerability_return"], 55, self.config.timers.vulnerability_return)
